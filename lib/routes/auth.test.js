...
describe('POST /api/auth', () => {
  ...
  describe('/signup', () => {
    ...
    it('should return a PasswordNotFoundError if password is blank', async () => {
      const res = await request(app).post(url).send({username: '', password: ''})
      const expected = {name: 'PasswordNotFoundError', message: 'Password not found'}
    ...
    describe('with a valid username and password', () => {
      it('should return a success message', async () => {
        const new_user = {username: 'abc', password: '123456a!'}
        const res = await request(app).post(url).send(new_user)
