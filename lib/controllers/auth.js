...
const login = async (req, res) => {
  ...
  if (!user) return errorHandler(errors.auth('Invalid username/password'), res, 401)

  try {
    const is_auth = await user.authenticatePassword(password)

    if (!is_auth) return errorHandler(errors.auth('Invalid username/password'), res, 401)

    res.cookie('jwt', createToken(user._id, user.username), {httpOnly: true, maxAge: maxAge * 1000})
...
const logout = (req, res) => {
  if (!req.cookies.jwt) return errorHandler(errors.auth('You are not logged in'), res, 401)
...
const signUp = async (req, res) => {
  ...
  try {
    const {username, _id} = await User.create(new_user)
